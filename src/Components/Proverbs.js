...
function Proverbs() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await axios.get('https://oral-lierature-api.onrender.com/api/v1/proverbs');
        setTwisters(response.data.proverb);
        console.log(response.data.proverb);
      } catch (error) {
        console.error(error);
      }
    }
